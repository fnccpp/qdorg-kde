	...
	function deserializeTodoModel(s) {
		...
		for (var j = 0; j < lines.length; j++) {
			...
			if (newItem) {
				...
				var taskIndex = line.indexOf('[')
				
				if (taskIndex >= 0) {
					todoItem.status = (line[taskIndex + 1] == 'x') ? 'completed' : 'needsAction'
					todoItem.title = line.substr(taskIndex + 'x] '.length + 1)
