...
MouseArea {
	...
	function deleteItem() {
		textArea.text = ''
		exec("sed -i '" + (index+1) + "s/.*/\*\[ \]/' ~/.local/share/plasma_notes/todolist")  //+ plasmoid.configuration.noteFilename)
		//todoModel.removeItem(index) //RIMOSSO MODIFICATO
	}
	...
	RowLayout {
		...
		Item {
			id: indentItem
			Layout.preferredWidth: textArea.height * model.indent
		...
		MouseArea {
			...
			implicitWidth: textArea.implicitHeight
		...
		PlasmaComponents3.Label {
			id: lblHours
			text: (index+8)>9 ? index + 8 : '0' + (index + 8)
			font.pixelSize: 16
			property int size: 30 * units.devicePixelRatio
			Layout.minimumWidth: size
			Layout.minimumHeight: size
		...
		PlasmaComponents3.ToolButton {
			...
			Layout.preferredWidth: textArea.implicitHeight
